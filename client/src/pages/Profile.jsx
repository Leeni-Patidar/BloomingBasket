...
const Profile = () => {
  ...
  const [editingAddressId, setEditingAddressId] = useState(null); // null = add mode
    ...
    <div className="container mx-auto p-4">
      <div className="flex gap-8">
        ...
        <aside className="w-1/4 space-y-2">
          <button onClick={() => setActiveTab("personal")} className="block w-full py-2 text-left hover:bg-gray-200">Personal Info</button>
          <button onClick={() => setActiveTab("address")} className="block w-full py-2 text-left hover:bg-gray-200">Address</button>
          <button onClick={() => setActiveTab("password")} className="block w-full py-2 text-left hover:bg-gray-200">Change Password</button>
        </aside>
        ...
        <main className="w-3/4">
          ...
          {activeTab === "personal" && (
            <form onSubmit={handleProfileSubmit} className="space-y-4">
              ...
              <input type="text" value={profileData.name} disabled className="input bg-gray-100 cursor-not-allowed w-full" />
              <input type="email" value={profileData.email} disabled className="input bg-gray-100 cursor-not-allowed w-full" />
              <input type="text" value={profileData.phone} onChange={(e) => setProfileData({ ...profileData, phone: e.target.value })} placeholder="Phone" className="input w-full" />
          ...
          {activeTab === "address" && (
            <div className="space-y-6">
              ...
              <div className="flex gap-2">
                <button onClick={handleAddressSubmit} className="btn">{editingAddressId ? "Update Address" : "Add Address"}</button>
                {editingAddressId && (
                  <button onClick={resetAddressForm} className="btn bg-gray-300 text-black">Cancel</button>
                )}
              </div>
